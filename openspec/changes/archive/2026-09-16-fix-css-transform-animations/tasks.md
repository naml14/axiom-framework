...
- [x] 5.1 RED→GREEN: Add `fill-mode: both` spec scenario test in `tests/commit.test.ts` — verifies composed transform embeds `var(--animation-transform)` so CSS can combine Axiom translate + animation value at computed-style level.
- [x] 5.2 RED→GREEN: Add `fill-mode: forwards` spec scenario test in `tests/commit.test.ts` — verifies Axiom layout translate is preserved and updated after animation completes (post-animation re-render preserves `var(--animation-transform)`).
- [x] 5.3 RED→GREEN: Replace shallow `createApp` test with end-to-end wiring proof in `tests/commit.test.ts` — uses synchronous capturing scheduler + closure-variable layout change to drive the full `createApp(opts) → performUpdate → applyOps(commitOpts) → onTransformConflict` path.
