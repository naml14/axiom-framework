...
- Tag mismatch → add warning / increment mismatch counter
- Text mismatch → add warning / increment mismatch counter
- Missing portal marker → warn, skip portal
- Attribute mismatch → currently not patched during hydration (validate-only)
...
| ---------- | -------------- | ----------------- |
| Tag name mismatch (`div` vs `span`) | warn + count mismatch | throw |
| Text content mismatch | warn + count mismatch | throw |
| Children count mismatch | warn + count mismatch | throw |
| Attribute value mismatch | currently not validated in v0.2.7 | n/a |
| Marker not found in DOM | warn + count mismatch | throw |

After `commitHydrate` returns in soft mode, `result.mismatchCount` reflects the
number of detected divergences. A non-zero count indicates the SSR HTML
...
Errors are surfaced through two channels:

1. `onError(err, context)` callback receives phase/context metadata.
2. In strict mode, hydration mismatches throw plain `Error` (no wrapper).
...
const app = createApp(Component, root, {
  ...
  strictHydration: true,
  onError(err, context) {
    console.error(`[axiom] error in phase: ${context.phase}`, err)
  },
})
...
try {
  ...
  // strict mismatch throws plain Error
  console.error(err)
}
```
