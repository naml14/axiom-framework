...
**Compliance summary**: 5/5 scenarios compliant. Existing 562 tests acting as regression suite also passed.
...
| `src/app.ts` | ✅ Implemented | Calls `releaseLayoutResult(layout)` cleanly after commit phase. |
...
| Release after commit | ✅ Yes | Executed in `app.ts` directly after `commitFull`/`commitHydrate`. |
